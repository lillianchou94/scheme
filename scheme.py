...
def do_lambda_form(expressions, env):
    ...
    exp = LambdaProcedure(formals, body, env)
    return exp
...
def do_if_form(expressions, env):
    ...
    elif scheme_false(predicate) and len(expressions) == 3:
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 1:
        return scheme_eval(expressions[0], env, True)
...
def make_let_frame(bindings, env):
    ...
    for x in bindings:
        check_form(x, 2, 2)
        formals += [x.first]
        vals += [scheme_eval(x.second.first, env)]
    ...
    formals = list_to_pairs(formals)
    vals = list_to_pairs(vals)
    check_formals(formals)
...
def check_formals(formals):
    ...
    def well_formed(lst):
        if lst is nil:
            return True
        elif scheme_symbolp(lst.first):
            return well_formed(lst.second)
        return False

    if not no_repeats(formals):
        raise SchemeError("repeating formal parameters")
    elif not well_formed(formals):
        raise SchemeError("parameters not well formed")
